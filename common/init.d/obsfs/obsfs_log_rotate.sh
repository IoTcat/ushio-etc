...
function compress()
{
    ...
    do
        ...
        need_tar="false"
        # have T, need tar
        files=`find ./ -maxdepth 1 -name "obsfs_log*" | grep -v ".old." | grep -v ".tar.gz" | grep "T"`
        if [ -n "${files}" ];then
        ...
        if [ -n "${files_no_T}" ];then
            for file in ${files_no_T}
            do
                curr_file_pid=$(echo ${file} | awk -F 'obsfs_log' '{print $2}')
                check_pid_exist=$(ps -ef | grep -v grep | awk '{print $2}' | grep ${curr_file_pid})
